...
import {
  getRefinedRegion, getDashboardPlantName, getIndustrialZone, getApproximateCoordinates, REGION_COUNTIES
} from '../utils/hydrogenHelpers';
import { MOCK_SUPPLY_MATRIX, MOCK_DEMAND_MATRIX, COLORS_PROCESS, COLORS_USAGE } from '../utils/constants';
import { fetchHydrogenRecords } from '../lib/fetchHydrogenRecords';
...
const HydrogenDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const { supplyData: localSupply, demandData: localDemand, rawSupply, rawUsage } = await fetchHydrogenRecords();

        setRawData({ supply: rawSupply, demand: rawUsage });

        if (localSupply.length === 0 && localDemand.length === 0) throw new Error("Supabase 沒有資料");
                            ...
                            <ErrorBoundary>
                                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                                    <ScatterChart margin={{top:30, right:30, bottom:30, left:20}}>
                                        ...
                                        <YAxis type="number" dataKey="intensity" name="強度" unit="kg/kg" domain={[0, 16]} tick={{fontSize:10, fill:'#64748b'}}>
